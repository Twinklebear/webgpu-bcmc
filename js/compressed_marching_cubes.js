  ...
  this.decompressBlocksPipeline = device.createComputePipeline({
    layout: device.createPipelineLayout({
      bindGroupLayouts: [
        ...
        this.decompressBlocksStartOffsetBGLayout,
      ],
    }),
    compute: {
      module: device.createShaderModule({
  ...
  this.computeBlockHasVerticesPipeline = device.createComputePipeline({
    layout: device.createPipelineLayout({
      bindGroupLayouts: [
        ...
        this.computeBlockHasVerticesBGLayout,
      ],
    }),
    compute: {
      module: device.createShaderModule({
...
CompressedMarchingCubes.prototype.reportMemoryUse = function () {
  ...
  var memUse = {
    mc: {
      ...
      activeBlockOffsets:
        this.scanPipeline.getAlignedSize(this.totalBlocks) * 4,
      ...
      blockHasVertsOffset: this.scanPipeline.getAlignedSize(
        this.numActiveBlocksStorage
      ),
      blocksWithVertices: this.numBlocksWithVerticesStorage * 4,
      blockVertexOffsets: this.scanPipeline.getAlignedSize(
  ...
  for (const prop in memUse.cache) {
    ...
    cacheText +=
      "<li>" + prop + ": " + formatBytes(memUse.cache[prop]) + "</li>";
  }
  cacheText += "</ul>";
  return [mcText, cacheText, formatBytes(totalMem)];
};

CompressedMarchingCubes.prototype.setCompressedVolume = async function (
  volume,
  compressionRate,
  volumeDims,
  volumeScale
...
CompressedMarchingCubes.prototype.computeSurface = async function (
  isovalue,
  perfTracker
) {
  console.log(`=====\nIsovalue = ${isovalue}`);
  ...
  if (numActiveBlocks > this.numActiveBlocksStorage) {
    this.numActiveBlocksStorage = Math.ceil(
      Math.min(
        this.totalBlocks,
        Math.max(numActiveBlocks, this.numActiveBlocksStorage * 1.5)
      )
    );
    var scanAlignedSize = this.scanPipeline.getAlignedSize(
      this.numActiveBlocksStorage
    );
    ...
    this.blockWithVerticesScanner = this.scanPipeline.prepareGPUInput(
  ...
  var numBlocksWithVertices = await this.blockWithVerticesScanner.scan(
  ...
  if (numBlocksWithVertices > this.numBlocksWithVerticesStorage) {
    this.numBlocksWithVerticesStorage = Math.floor(
      Math.min(
        this.totalBlocks,
    ...
    this.blockVertexOffsetsScanner = this.scanPipeline.prepareGPUInput(
      this.blockVertexOffsets,
      scanAlignedSize
    );
  }
  ...
  if (numVertices > this.numVerticesStorage) {
    this.numVerticesStorage = Math.floor(
      Math.max(numVertices, this.numVerticesStorage * 1.5)
    );
...
CompressedMarchingCubes.prototype.decompressBlocks = async function (
  nBlocksToDecompress,
  decompressBlockIDs
) {
...
CompressedMarchingCubes.prototype.computeBlockVertexCounts = async function () {
  ...
  var total = await this.blockVertexOffsetsScanner.scan(
    this.numBlocksWithVertices
  );
  return total;
};
...
CompressedMarchingCubes.prototype.computeVertices = async function () {
  ...
  for (var i = 0; i < numChunks; ++i) {
    var numWorkGroups = Math.min(
      this.numBlocksWithVertices - i * this.maxDispatchSize,
      this.maxDispatchSize
    );
    var commandEncoder = this.device.createCommandEncoder();
    var pass = commandEncoder.beginComputePass();
