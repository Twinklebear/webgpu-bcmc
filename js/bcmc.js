(async () => {
  ...
  var compressedData = await fetch("/models/" + zfpDataName).then((res) =>
  ...
  var displayCacheInfo = function () {
    var percentActive =
      (compressedMC.numActiveBlocks / compressedMC.totalBlocks) * 100;
  ...
  while (true) {
    ...
    if (isovalueSlider.value != currentIsovalue || requestRecompute) {
      ...
      var start = performance.now();
      totalVerts = await compressedMC.computeSurface(
        currentIsovalue,
        perfResults
      );
      var end = performance.now();
